...
//
// 第一层：runGateway() 顶部
// 这里看“配置、日志、provider registry、workspace、tool registry、store”
//
// 第二层：中段 wiring
// 这里看“provider、skills、memory、subagent、cron、heartbeat、sessions”
//
// 第三层：最后 channel + gateway server 启动
// 这里看 Telegram 等通道是在哪一步接进来的
//
// 对你现在的目标来说，这份文件最值得看的不是全部细节，
// 而是它的“组合根”角色：
//
// - 它不负责某一个具体业务点
// - 它负责把各个子系统装配到一起
//
// 后面你真要在 openlaw-go 里复刻 goclaw，runGateway() 这种组合根一定会回来。
//
import (
	"context"
	"encoding/json"
