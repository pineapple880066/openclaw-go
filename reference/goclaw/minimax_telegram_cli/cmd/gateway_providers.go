...
// Source: /Users/pineapple/Desktop/OpenClaw_go/goclaw/cmd/gateway_providers.go
// Scope: MiniMax provider, Telegram channel, and CLI bootstrap.
// This file is intentionally excluded from the openlaw-go build.

package cmd

// 这份文件是“provider 从配置进入系统”的真实入口。
//
// 对你现在最重要的是 MiniMax 这一段：
//
// - 它不是一个独立 Provider 类型
// - 它注册时仍然调用 NewOpenAIProvider(...)
// - 只是额外加了一次 WithChatPath("/text/chatcompletion_v2")
//
// 也就是说：
//
// 1. OpenAIProvider 负责通用协议层
// 2. gateway_providers.go 负责按具体厂商参数把它实例化
//
// 读这份文件时，重点看：
//
// - registerProviders(...)
// - cfg.Providers.MiniMax.APIKey != "" 那一段
// - registerProvidersFromDB(...) 这类“配置来源不同，但最终都要注册到 Registry” 的思路
//
import (
	"context"
	"encoding/json"
