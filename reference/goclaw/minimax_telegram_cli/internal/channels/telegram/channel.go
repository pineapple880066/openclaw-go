...
// Source: /Users/pineapple/Desktop/OpenClaw_go/goclaw/internal/channels/telegram/channel.go
// Scope: MiniMax provider, Telegram channel, and CLI bootstrap.
// This file is intentionally excluded from the openlaw-go build.

package telegram

// 这份文件是 Telegram 通道的真正入口。
//
// 它回答的是：
//
// 1. Telegram bot 在 goclaw 里怎么初始化
// 2. long polling 怎么启动
// 3. Channel 这个对象到底保存了哪些运行时状态
//
// 读这份文件时，建议先抓 4 个点：
//
// - Channel 结构体字段
...
// - StreamEnabled(...)
//
// Channel 结构体本身就很能说明问题：
//
// - bot 是 Telegram Bot API 客户端
// - pairingService / agentStore / configPermStore / teamStore 是外部依赖
// - placeholders / reactions / threadIDs / approvedGroups 这些是 Telegram 自己的会话状态
//
// 这也是为什么 Telegram 通道在 goclaw 里不是一个薄薄的 webhook handler，
// 而是一个真正的子系统。
//
import (
	"context"
