...
// Source: /Users/pineapple/Desktop/OpenClaw_go/goclaw/internal/channels/telegram/handlers.go
// Scope: MiniMax provider, Telegram channel, and CLI bootstrap.
// This file is intentionally excluded from the openlaw-go build.

package telegram

// 这份文件是 Telegram 入站消息处理主链。
//
// 你后面如果只想知道：
// “Telegram 一条消息进来后，goclaw 是怎么处理的？”
// 那就先看这份。
//
// 阅读顺序建议：
//
// 1. handleMessage(...)
// 2. 里面对 service message / group / mention / media / command 的分支
// 3. 最后它怎么把消息推到 bus / session / agent 链路
//
// 这份文件体现的不是 Telegram SDK 用法本身，
// 而是 goclaw 自己的消息治理策略：
//
// - 哪些消息直接跳过
// - 哪些消息要做 mention gate
// - 哪些命令要优先截获
// - 哪些消息要进入后面的 agent loop
//
import (
	"context"
	"fmt"
